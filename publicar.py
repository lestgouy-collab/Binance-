...
# Nuevos formatos de texto totalmente diferentes
opciones = [
    (
        f"📊 [INFORME FLASH DE MERCADO]\n\n"
        f"Movimientos clave detectados en la sesión actual:\n"
        f"• {t1[1]} ({t1[2]}): ${val1:,.4f} ({pct1:+.2f}%)\n"
        f"• {t2[1]} ({t2[2]}): ${val2:,.4f} ({pct2:+.2f}%)\n\n"
        f"¿Cuál de estos dos activos liderará el próximo movimiento alcista? Coméntalo 🚀\n\n"
        f"#BinanceSquare #Crypto #Trading {t1[2]} {t2[2]}"
    ),
    (
        f"💡 [RADAR DE OPORTUNIDADES]\n\n"
        f"Analizando la fuerza relativa de las criptomonedas:\n"
        f"- {t1[2]} cotiza en ${val1:,.4f} con una variación de {pct1:+.2f}%\n"
        f"- {t2[2]} se posiciona en ${val2:,.4f} con un cambio de {pct2:+.2f}%\n\n"
        f"Mantén tu estrategia clara y opera con responsabilidad. ¿Estás posicionado? 📈\n\n"
        f"#BinanceSquare #Altcoins #MercadoCripto {t1[2]} {t2[2]}"
    )
]
